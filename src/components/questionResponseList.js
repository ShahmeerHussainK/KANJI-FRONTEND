...
import { url,logout, media_url } from './helper'
import { Button, Modal, ModalHeader, ModalBody, ModalFooter } from 'reactstrap';
import Calendar from 'react-calendar';
import Fade from 'react-reveal/Fade';
// import { Calendar } from 'react-native-calendars';
// import {loadingGIF} from '../CSS/assets-minified/giphy_loading.gif' 
...
class QuestionResponseList extends Component{
    ...
    refreshAfterSearch = () =>{
        ...
        http.open('GET', url+'/api/questions/responses?user_id='+user_id+'&page='+1+'&page_size='+this.state.page_size, true);
        http.setRequestHeader("Authorization", "Token "+token);
        http.onreadystatechange = function(){
            if(http.readyState === 4 && http.status === 200){
                var json = JSON.parse(http.responseText)
    ...
    HandleToDateSelect = (ToDate) =>{
        ...
        this.setState({
            ...
            //("after setting"); //(this.state.ToDate)
        })
    }
    ...
    triggerFileDownload = () => {
     ...
     downloadFile = () =>{
         ...
         var from = ""+this.state.FromDate.getFullYear()
         from = this.state.FromDate.getMonth()+1 < 10 ? from+"-0"+(this.state.FromDate.getMonth()+1) : from+"-"+(this.state.FromDate.getMonth()+1)
         from = this.state.FromDate.getDate() < 10 ? from+"-0"+this.state.FromDate.getDate() : from+"-"+(this.state.FromDate.getDate())
         var to = ""+this.state.ToDate.getFullYear()
         to = this.state.ToDate.getMonth()+1 < 10 ? to+"-0"+(this.state.ToDate.getMonth()+1) : to+"-"+(this.state.ToDate.getMonth()+1)
         to = this.state.ToDate.getDate() < 10 ? to+"-0"+this.state.ToDate.getDate() : to+"-"+(this.state.ToDate.getDate())
         //(from)
         //(to)
         const cookies = new Cookies();
        var http = new XMLHttpRequest();
        var dashboard_ref = this
        var user_id = cookies.get('user_id')
        var token = cookies.get('token')
        dashboard_ref.setState({
            fileProcessing:true
        })
        // document.getElementById("bod").className = "bodPro"

        http.open('POST', url+'/api/questions/responses?user_id='+user_id, true);
        http.setRequestHeader("Authorization", "Token " + token);
        ...
        http.onreadystatechange = function(){
            if(http.readyState === 4 && http.status === 200){
                ...
                console.log(json)
                if(json.status === 200){
                    document.getElementById("file").setAttribute("href",media_url+json.link)
                    dashboard_ref.triggerFileDownload()
                ...
                if(json.status === 403){
                    dashboard_ref.setState({
                        SelectDateModalOpen:false,
                        messageBoxOpen:true,
                        messageTitle:"Response Limit Exceeded",
                        messageBody:"Only 1 million records can be downloaded at a time.Try selecting different dates",
                        fileProcessing:false
                        })
                }
                if(json.status === 204){
                    dashboard_ref.setState({
                        SelectDateModalOpen:false,
                        messageBoxOpen:true,
                        messageTitle:"Response Record Download",
                        messageBody:"There are no record. Try selecting different dates",
                        fileProcessing:false
                        })
                }
            }
        ...
        return(
            <div >
                <div className="clearfix"  id="page-header">
                    ...
                    <div >
                        <SideBar isActive="response"/>
                    </div>
                    <div id="page-content-wrapper" className="rm-transition">
                    <div id="page-content" >
                   

                    <div class="modalDownloading" hidden={!this.state.fileProcessing}>
                    <p className="processingText">Preparing File For Download...</p>
                    </div>
                    ...
                    <Modal size="lg" isOpen={this.state.SelectDateModalOpen}>
                        ...
                        <ModalFooter>
                            <Button onClick={this.closeSelectDateModal}>Cancel</Button>
                            <Button color="primary" onClick={this.downloadFile}>Download</Button>
                        </ModalFooter>
                    </Modal>
                    <div className="row">
                    ...
                    <div className="form-group">
                        ...
                        <a href="" id="file" hidden="true">Down Test</a>
                        </div>
                    </div>
