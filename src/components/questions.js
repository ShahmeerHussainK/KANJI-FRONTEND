    ...
    componentWillMount = () =>{
        ...
        http.onreadystatechange = function (){
            if (http.readyState === 4 && http.status === 200) {
                var json = JSON.parse(http.responseText)
                        //(http.responseText)
                        if(dashboar_ref.state.branch_admin === 'true' || dashboar_ref.state.branch_user === 'true'){
                            if (json.status === 200) {
                                //(json.branches)
                              var branches = []
                              var customer = []
                            ...
                            if (json.status === 200) {
                              ...
                              var customer = []
                                  ...
                                  //('never here')
                                  json.organisations && json.organisations.forEach(element => {
                                    var temp = {
                            ...
                            dashboar_ref.setState({options_organization:orgs, loading:false,
                                ...
                                //(dashboar_ref.state.options_organization)
                            })
                            }
                            else{
                                dashboar_ref.setState({
        ...
        fetch = () =>{
            ...
            const cookies = new Cookies();
                ...
                var local_url = url+'/api/questions?user_id='+user_id+'&org_id='+this.state.organization.id+'&branch_id='+this.state.branch.id+'&customer_code='+this.state.customer+'&page='+this.state.activePage+'&page_size='+this.state.page_size
                //(url)
            ...
            http.onreadystatechange = function () {
                ...
                if(json.status === 200){
                    ...
                    //("count "+json.total_questions)
                    dashboar_ref.setState({questions:question,
                    total_records:json.total_questions,tableHidden:false,loading:false,activePage:1,tableLoading:false, activePage:dashboar_ref.state.activePage})
                }
                else{
                    dashboar_ref.setState({
                       error_modal_open:true,
        ...
        return(
            <div>
                ...
                <ReactCSSTransitionGroup
                     ...
                     transitionEnterTimeout={5000}
                        transitionLeaveTimeout={3000}>
                <QuestionTable data={this.state.questions} record={this.state.total_records} fetch={this.next_page} activePage={this.state.activePage} orgs={this.state.options_organization} branches={this.state.options_branch} section={1} refresh={this.fetch} hideCTA={true}/>
                </ReactCSSTransitionGroup>
                </div>
                </div>
                </div>
