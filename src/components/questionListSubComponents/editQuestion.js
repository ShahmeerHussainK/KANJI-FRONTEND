...
class EditQuestion extends Component {
    ...
    componentWillMount = () => {
        ...
        var superuser = cookies.get('superuser')
            ...
            var http_branch = new XMLHttpRequest();
        if (this.state.already_mounted === false) {
            ...
            //("00000000000000000")
            //(dashboar_ref.props.data.organisation)
            //(dashboar_ref.props.data.branch)
            http_branch.open('GET', url+'/api/questions/wizard/runsheet?user_id='+ user_id+'&org_id='+dashboar_ref.props.data.organisation.id+'&branch_id='+dashboar_ref.props.data.branch.id, true)
                ...
                if (http.readyState === 4 && http.status === 200) {
                    var json = JSON.parse(http.responseText)
                        if (json.status === 200) {
                            ...
                            //(dashboar_ref.state.true_selected_status_type)
                            if(dashboar_ref.props.data.true_action_status === true){
                                var true_list = dashboar_ref.state.true_selected_status_type.id === 0 ? drop_status_codes :run_status_codes
                            }
                            ...
                            json.question_log_types.forEach(element =>{
                                ...
                                //(element.log_type_id)
                                //(dashboar_ref.props.data.true_action_log_type)
                                log_types.push(temp)
                            })
                            //("*************************")
                            //(run_status_codes)
                            ...
                            //("^^^^^^^^^^^^")
                            //(dashboar_ref.state.drop_status_list)
                            //(dashboar_ref.state.run_status_list)
                            }
                        }
                        else{
                            dashboar_ref.setState({
    ...
    next_section_two = () => {
        ...
        if(this.state.customer === "all"){
            base_url = base_url + '' + '&customer_code='
        }
        else{
    ...
    handle_false_log_checkbox_click = () => {
        if(this.state.action.label === "status" || this.state.action.label === "text" || this.state.action.label === "page" || this.state.action.label === "log" || this.state.false_block === true){
            return
        }
    ...
    handle_false_status_status_checkbox_click = () => {
        if(this.state.action.label === "text" || this.state.action.label === "page" || this.state.action.label === "log" || this.state.false_block === true){
            return
        }
        if (this.state.false_No_Action === false) {
    ...
    update_question = () => {
        ...
        if (this.state.true_block === true & (this.state.true_block_text === null || this.state.true_block_text === "")) {
            this.setState({
        ...
        if (this.state.false_block === true & (this.state.false_block_text === null || this.state.false_block_text === "")) {
                                        ...
                                        <div className="row form">
                                            <div className="col-md-12">
                                                <div class="inputGroup" onClick={this.handle_false_take_photo_checkbox_click}>
                                                    <input type="checkbox" id="false_log_checkbox" checked={this.state.false_take_photo} disable={this.state.false_take_photo_disabled} onClick={this.handle_false_take_photo_checkbox_click} class="custom-checkbox"></input>
                                                    <label>
                                                        <b className="label-text">Take Photo</b>
                                                    </label>
                                                </div>
